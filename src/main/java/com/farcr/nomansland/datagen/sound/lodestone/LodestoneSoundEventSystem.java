...
public abstract class LodestoneSoundEventSystem extends SoundDefinitionsProvider {
    ...
    public static SoundDefinition.Sound sound(String name, SoundDefinition.SoundType type) {
        return sound(name.contains(":") ? ResourceLocation.parse(name) : ResourceLocation.fromNamespaceAndPath(INSTANCE.modId, name), type);
    }

    public SoundDefinition.Sound[] sounds(String name, int variants) {
        return sounds(name, SoundDefinition.SoundType.SOUND, variants);
    }

    public SoundDefinition.Sound[] sounds(String name, SoundDefinition.SoundType type, int variants) {
        ...
        for (int i = 0; i < variants; i++) {
            ...
            sounds[i] = sound(resourceLocation, type);
        }
        return sounds;
    }
