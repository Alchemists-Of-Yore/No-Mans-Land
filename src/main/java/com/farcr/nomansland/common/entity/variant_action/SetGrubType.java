...
public class SetGrubType implements VariantActionType {
    private boolean resolved;
    private EntityType<?> growsInto;

    public EntityType<?> getGrowsInto() {
        return resolved ? growsInto : null;
    }

    @Override
    public VariantActionType resolve(JsonObject actionsArgs, JsonObject variantArgs, JsonObject defaultArgs) {
        SetGrubType result = new SetGrubType();
            ...
            if (element.isJsonPrimitive()) {
                ...
                if (!id.isEmpty()) {
                    ...
                    if (location != null && BuiltInRegistries.ENTITY_TYPE.containsKey(location)) {
                        result.growsInto = BuiltInRegistries.ENTITY_TYPE.get(location);
                    }
                }
            }
        }
