...
public record CaveRiverTestDensityFunction(DensityFunction riverRadius, WatershedMap watershedMap) implements DensityFunction {
    public static final KeyDispatchDataCodec<CaveRiverTestDensityFunction> CODEC = KeyDispatchDataCodec.of(
            MapCodec.unit(new CaveRiverTestDensityFunction(DensityFunctions.constant(20), null))
    );
    ...
    private double shorelineDensity(River.RiverSpaceCoordinates river, int blockX, int blockY, int blockZ, double riverRadius) {
        ...
        double riverDepthMultiplier = Mth.clampedMap(verticalDistance, 2, -3, 1, 3);
    ...
    public DensityFunction mapAll(Visitor visitor) {
        return visitor.apply(new CaveRiverTestDensityFunction(this.riverRadius().mapAll(visitor), this.watershedMap()));
    }
