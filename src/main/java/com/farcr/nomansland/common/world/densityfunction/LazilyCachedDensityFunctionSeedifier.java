...
public abstract class LazilyCachedDensityFunctionSeedifier implements DensityFunction.Visitor {
    private static final ThreadLocal<Map<WorldGeneratorEntry, LazilyCachedDensityFunctionSeedifier>> visitorCache =
    ...
    private final ThreadLocal<Map<ResourceKey<NormalNoise.NoiseParameters>, DensityFunction.NoiseHolder>> holderCache;

    public LazilyCachedDensityFunctionSeedifier() {
        this.holderCache =  ThreadLocal.withInitial(HashMap::new);
    }
    ...
    public DensityFunction.NoiseHolder visitNoise(DensityFunction.NoiseHolder noiseHolder) {
        ...
        return this.holderCache.get().computeIfAbsent(noiseHolder.noiseData().unwrapKey().orElseThrow(), (resourceKey) -> modifyNoiseHolder(noiseHolder, noiseParameters));
    }
